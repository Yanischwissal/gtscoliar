...
{
    public partial class frmaddnote : DevExpress.XtraEditors.XtraForm
    {
        public frmaddnote()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var reqe = (from evalue in shared.db.evaluers
                       ...
                       select new { numrostg = stg.numstg, nom = stg.nom, control1 = evalue.controle1, control2 = evalue.controle2, exm = evalue.exm, rtp = evalue.ratrapag }).ToList();

            if (int.Parse(reqe.Count().ToString()) > 0)
            {
               
                
                gridControl1.DataSource = reqe.ToList();
            ...
            else
            {
                ...
                df = gridControl1.DataSource as DataTable;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataRow row in df.Rows )
                {
                    ...
                    ev.stagiair = row["numrostg"].ToString();
                    ev.semastre = comboBox4.SelectedValue.ToString();
                    ev.controle1 = float.Parse(row["control1"].ToString());
                    ev.controle2 = float.Parse(row["control2"].ToString());
                    ev.exm = float.Parse(row["exm"].ToString());
                    ev.ratrapag = float.Parse(row["rtp"].ToString());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            var reqe = from evalue in shared.db.evaluers
                        join stg in shared.db.stgs on evalue.stagiair equals stg.numstg
                        where   evalue.module == idmod && evalue.semastre == idsem && evalue.anne_scoliar == 1
            ...
            var rq = from stg in shared.db.stgs
                     where stg.numrosection == sec
                     select reqe;

            gridControl1.DataSource = rq.ToList();
        }
    }
}
